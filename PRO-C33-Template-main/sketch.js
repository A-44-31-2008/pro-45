...
function setup() {
  ...
  bubble = createSprite(290,340,20,20);
  bubble.addImage(bubble_img);
  bubble.scale = 0.1;

  p1 = createSprite(290,460,20,20);
  p1.addImage(p1Img);
  p1.scale = 0.25;
  
  p2 = createSprite(100,200,20,20);
  p2.addImage(p1Img);
  p2.scale = 0.25;
  //bunny sprite
  blink.frameDelay = 20;
  ...
  b=createImg('balloon.png');
  b.position(10,250);
  b.size(150,100);
  b.mouseClicked(airBlow);
...
function draw() 
{
  ...
  if(collide(fruit,bubble,40) == true)
    ...
    if(collide(fruit,p1,0) == true)
    {
      p2.position.x=fruit.position.x;
      p2.position.y=fruit.position.y;
